...
class TestFITGeneration:
    ...
    def test_duration_matches_workout(
        self, sample_workout: dict, sample_profile: dict, tmp_path: Path
    ) -> None:
        path = str(tmp_path / "test.fit")
        result = generate_fit(
...
class TestHRSamples:
    def test_with_hr_samples(
        self, sample_workout: dict, sample_profile: dict, tmp_path: Path
    ) -> None:
        path = str(tmp_path / "hr.fit")
        hr = [90, 95, 100, 105, 110, 115, 120, 115, 110, 100]
        result = generate_fit(
            sample_workout, hr_samples=hr, output_path=path, profile=sample_profile
        )
        assert result["hr_samples"] == len(hr)
        assert result["avg_hr"] > 0

    def test_no_hr_uses_default(
        self, sample_workout: dict, sample_profile: dict, tmp_path: Path
    ) -> None:
    ...
    def test_hr_changes_calories(
        ...
        r1 = generate_fit(
            sample_workout, hr_samples=low_hr, output_path=path1, profile=sample_profile
        )
        r2 = generate_fit(
            sample_workout, hr_samples=high_hr, output_path=path2, profile=sample_profile
        )
...
class TestEdgeCases:
    ...
    def test_missing_start_time_raises(self, sample_profile: dict, tmp_path: Path) -> None:
        ...
        with pytest.raises(ValueError, match="missing valid start/end time"):
            generate_fit(
                workout,
                hr_samples=None,
                output_path=str(tmp_path / "x.fit"),
    ...
    def test_exercise_with_zero_sets(self, sample_profile: dict, tmp_path: Path) -> None:
        ...
        result = generate_fit(
            workout,
            hr_samples=None,
            output_path=str(tmp_path / "nosets.fit"),
            profile=sample_profile,
        )
        assert result["total_sets"] == 0
    ...
    def test_isometric_exercise(self, sample_profile: dict, tmp_path: Path) -> None:
        ...
        result = generate_fit(
            workout, hr_samples=None, output_path=str(tmp_path / "iso.fit"), profile=sample_profile
        )
        assert result["total_sets"] == 1
    ...
    def test_cardio_exercise(self, sample_profile: dict, tmp_path: Path) -> None:
        ...
        result = generate_fit(
            workout, hr_samples=None, output_path=fit_path, profile=sample_profile
        )
        assert result["total_sets"] == 1
        ...
        assert any(d >= 5000.0 for d in distances), (
            f"Expected total_distance >= 5000 in FIT, got {distances}"
        )
    ...
    def test_cardio_uses_set_duration(self, sample_profile: dict, tmp_path: Path) -> None:
        ...
        result = generate_fit(
            workout, hr_samples=None, output_path=str(tmp_path / "bike.fit"), profile=sample_profile
        )
        assert result["total_sets"] == 1
    ...
    def test_special_chars_in_title(self, sample_profile: dict, tmp_path: Path) -> None:
        ...
        workout = {
            "id": "emoji",
            "title": "💪 André's Push/Pull & Legs Day",
            "start_time": "2026-04-01T20:00:00+00:00",
            "end_time": "2026-04-01T20:30:00+00:00",
            "exercises": [
                {
        ...
        result = generate_fit(
            workout,
            hr_samples=None,
            output_path=str(tmp_path / "emoji.fit"),
            profile=sample_profile,
        )
        assert result["exercises"] == 1
    ...
    def test_very_long_workout(self, sample_profile: dict, tmp_path: Path) -> None:
            ...
            "exercises": [
                {
                    ...
                    "exercise_template_id": "X",
                    "sets": [
                        {"index": i, "type": "normal", "weight_kg": 100, "reps": 5}
                        for i in range(20)
                    ],
                }
            ],
        ...
        result = generate_fit(
            workout, hr_samples=None, output_path=str(tmp_path / "long.fit"), profile=sample_profile
        )
    ...
    def test_negative_weight_clamped(self, sample_profile: dict, tmp_path: Path) -> None:
        ...
        result = generate_fit(
            workout, hr_samples=None, output_path=str(tmp_path / "neg.fit"), profile=sample_profile
        )
        assert result["total_sets"] == 1
