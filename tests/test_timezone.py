...
from hevy2garmin.fit import _FIT_EPOCH_S, _tz_offset_seconds, generate_fit
...
class TestTzOffsetHelper:
    def test_summer_offset(self) -> None:
        ...
        assert (
            _tz_offset_seconds("Europe/Berlin", datetime(2026, 7, 1, tzinfo=timezone.utc)) == 7200
        )

    def test_winter_offset(self) -> None:
        # Same zone is CET (UTC+1) on 1 January — the helper must be DST-aware.
        assert (
            _tz_offset_seconds("Europe/Berlin", datetime(2026, 1, 1, tzinfo=timezone.utc)) == 3600
        )

    def test_negative_offset(self) -> None:
        # America/New_York is EDT (UTC-4) on 1 July.
        assert (
            _tz_offset_seconds("America/New_York", datetime(2026, 7, 1, tzinfo=timezone.utc))
            == -4 * 3600
        )
...
class TestFitLocalTimestamp:
    def test_absent_without_timezone(self, sample_profile: dict, tmp_path) -> None:
        ...
        generate_fit(
            _workout("2026-07-01T12:00:00+00:00", "2026-07-01T12:45:00+00:00"),
            None,
            out,
            profile=prof,
        )
    ...
    def test_dst_winter_offset_differs(self, sample_profile: dict, tmp_path) -> None:
        ...
        out = str(tmp_path / "w.fit")
        generate_fit(
            _workout("2026-01-01T12:00:00+00:00", "2026-01-01T12:45:00+00:00"),
            None,
            out,
            profile=prof,
        )
    ...
    def test_invalid_timezone_does_not_crash_and_omits(
        self, sample_profile: dict, tmp_path
    ) -> None:
        prof = dict(sample_profile)
